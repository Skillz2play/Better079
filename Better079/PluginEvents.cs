...
namespace Better079
{
    public class PluginEvents
    {
        ...
        public static List<Camera079> GetSCPCameras()
        {
            ...
            {
                if (player.GameObject.GetComponent<ReferenceHub>().characterClassManager.CurRole.team == Team.SCP && player.GameObject.GetComponent<ReferenceHub>().characterClassManager.NetworkCurClass != RoleType.Scp079)
                {
                    foreach (var cam in list)
                    {
                        if (Vector3.Distance(cam.transform.position, player.CameraTransform.position) <= Better079Plugin.instance.Config.b079_a1_dist)
                        {
                            cams.Add(cam);
                        }
                    }
        ...
        public static IEnumerator<float> GasRoom(Room room, ReferenceHub scp)
        {
            ...
            foreach (var item in doors)
            {
                ...
                item.ServerChangeLock(DoorLockReason.AdminCommand, true);
            }
            for (int i = Better079Plugin.instance.Config.b079_a2_timer; i > 0f; i--)
            {
            ...
            foreach (var item in doors)
            {
                item.ServerChangeLock(DoorLockReason.AdminCommand, false);
            }
        }
    }
}